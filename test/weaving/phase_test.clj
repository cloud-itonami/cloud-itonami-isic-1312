...
(deftest ^{:doc "Initial state should be idle."} initial-phase-is-idle
  (let [state {:phase :idle}]
    (is (= :idle (:phase state)))))

(deftest ^{:doc "Can transition to proposed phase."} transition-to-proposed
  (let [state {:phase :idle}
...
(deftest ^{:doc "Can transition to evaluated phase."} transition-to-evaluated
...
(deftest ^{:doc "Can transition to escalated phase."} transition-to-escalated
  (let [state {:phase :evaluated}
        new-state (phase/transition-to-escalated state)]
    (is (= :escalated (:phase new-state)))))

(deftest ^{:doc "Can transition to approved phase."} transition-to-approved
  (let [state {:phase :escalated}
        new-state (phase/transition-to-approved state)]
    (is (= :approved (:phase new-state)))))

(deftest ^{:doc "Can transition to rejected phase."} transition-to-rejected
  (let [state {:phase :evaluated}
...
(deftest ^{:doc "Can transition to executed phase."} transition-to-executed
...
(deftest ^{:doc "Can reset to idle phase."} transition-to-idle
